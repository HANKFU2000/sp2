...
class SafeQueue {
  ...
  private:
  std::mutex lock_;
  std::condition_variable que_cond_;
  std::queue<T> que_;
  std::condition_variable in_working_cond_;
  size_t in_working_;
};
...
void SafeQueue<T>::push(const T& value) {
  ...
  que_.push(value);
  que_cond_.notify_all();
}

template<typename T>
T SafeQueue<T>::pop() {
  std::unique_lock<std::mutex> lk(lock_);
  que_cond_.wait(lk, [=] { return !this->que_.empty(); });
  T ret = que_.front();
  que_.pop();
  in_working_ += 1;
  return ret;
...
template<typename T>
void SafeQueue<T>::dec_working() {
  std::unique_lock<std::mutex> lk(lock_);
  in_working_ -= 1;
  in_working_cond_.notify_one();
}

template<typename T>
bool SafeQueue<T>::empty() {
  std::unique_lock<std::mutex> lk(lock_);
  in_working_cond_.wait(lk, [=] { return this->que_.empty() && in_working_ == 0; });
  return true;
}

std::vector<std::string> dirs;
SafeHashMap theTable;
SafeQueue<std::string> workQ;
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';

    {
      ...
      std::unique_lock<std::mutex> lk(theTable.lock);
      theTable.table[file].push_back(name);
      // 2bii. if file name not already in table ...
      if (theTable.table.find(name) == theTable.table.end()) {
        // ... insert mapping from file name to empty list in table ...
        theTable.table.insert({name, {}});
        // ... append file name to workQ
        workQ.push( name );
      }
    }
...
FILE *fd) {
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = &theTable.table[name];
...
int main(int argc, char *argv[]) {
  ...
  size_t numThread = 2;
  char *crawler_thread = getenv("CRAWLER_THREADS");
  if (crawler_thread) {
    // printf("%s\n", crawler_thread);
    numThread = atoi(crawler_thread);
  }
  ...
  for (i = start; i < argc; i++) {
    ...
    {
      std::lock_guard<std::mutex> lock(theTable.lock);
      // 3a. insert mapping from file.o to file.ext
      if (theTable.table.find(obj) == theTable.table.end()) {
        theTable.table.insert( { obj, { argv[i] } } );
      }

      // 3b. insert mapping from file.ext to empty list
      if (theTable.table.find(argv[i]) == theTable.table.end()) {
        theTable.table.insert( { argv[i], { } } );
